...
void mins::OptionsGPS::load_i(const std::shared_ptr<ov_core::YamlParser> &parser, int i) {
  ...
  bool geometry_msg = false;
  parser->parse_external(f, "gps" + std::to_string(i), "is_geometry_msg", geometry_msg);
  is_geometry_msg.insert({i, geometry_msg});
...
void mins::OptionsGPS::print_i(int i) {
  ...
  PRINT1("\t\t- is_geometry_msg: %s\n", is_geometry_msg.at(i) ? "true" : "false");
}